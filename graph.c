	...
	for (i = 0; i < graph->nodenum - 1; i++) {
		...
		while (flag) {
			...
			if (isedg(*graph, path[i + k], path[i + 1])) {
				...
				if (k == 0 && (i + k - 2) >= 0) { //不走原来的路是否能回
					int x = i + k - 2;
					int y = locate(*graph, path[i]);
					CNode *pt = NULL;
					for (pt = graph->roadlist[i].first; pt != NULL; pt = pt->next) {
						while (x >= 0) {
							if (isedg(*graph, graph->roadlist[pt->index].data, path[x])) {
								break;
							} else {
								x--;
							}
